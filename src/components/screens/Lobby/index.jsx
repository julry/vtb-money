...
import {useMemo, useEffect, useState, useCallback, lazy} from 'react';
import NumberPicker from "./NumberPicker";
import { GAME_CELLS } from "./constants";
import { UnfinishedModal } from './UnfinishedModal';
import { CellModal, preloadCellModal } from '../../shared/modals/CellModal';
import { preload } from "../../../constants/screensComponents";
import { useCharacterPath } from './useCharacterPath';
import {preloadWeekContent, getCellContent} from './cellContent/index';
import { CURRENT_WEEK } from "../../../contexts/ProgressProvider";

const OnBoardingModal = lazy(() =>
    import('../../shared/modals/OnboardingModal').then((m) => ({ default: m.OnBoardingModal }))
);
const SettingsModal = lazy(() =>
    import('../../shared/modals/SettingsModal').then((m) => ({ default: m.SettingsModal }))
);
const TurnsInfoModal = lazy(() =>
    import('../../shared/modals/TurnsInfoModal').then((m) => ({ default: m.TurnsInfoModal }))
);
const FinishedTurnsModal = lazy(() =>
    import('./FinishedTurnsModal').then((m) => ({ default: m.FinishedTurnsModal }))
);
...
const Lobby = () => {
    ...
    const onComplete = useCallback(async (index) => {
        ...
        const cell = await getCellContent(currCell.id);

        setTimeout(() => {
            handleOpenModal({
                Component: <CellModal cell={{...cell, ...currCell}} isLuck={isLuck} />,
            })
        }, 800)
    }, []);
    ...
    useEffect(() => {
        ...
        preloadWeekContent(user.progressWeek ?? CURRENT_WEEK);
        ...
        if ('requestIdleCallback' in window) {
            idleId = requestIdleCallback(preloadSecondary, { timeout: 2800 });
        } else {
            secondaryTimeoutId = setTimeout(preloadSecondary, 1400);
        }

        return () => {
            if (idleId) cancelIdleCallback(idleId);
            if (secondaryTimeoutId) clearTimeout(secondaryTimeoutId);
        };
    }, []);
    ...
    const handleClose = async () => {
        ...
        setGameState({...lastCell, id: user.lastOpenedCell});
        const cell = await getCellContent(user.lastOpenedCell);

        handleOpenModal({
            Component: <CellModal cell={{...cell, ...lastCell}} />
        });
    ...
    const handleMakeTurn = (number) => {
        ...
        const nextCell = GAME_CELLS[cellIndex + number];
        preloadCellModal(nextCell?.type)
