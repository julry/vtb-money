...
export function useMapPan({
	...
	useLayoutEffect(() => {
		if (!centerCellId || !viewportRef.current) return;

		const { clientWidth: vw, clientHeight: vh } = viewportRef.current;
		if (vw === 0 || vh === 0) return; // ещё не измерили

		const instant = !hasCenteredOnce.current;
		centerOnCellRef.current(centerCellId, { instant });
	}, [centerCellId, mapW, mapH]);
